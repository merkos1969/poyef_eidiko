...
(function(){
  ...
  function fiveCountFor(k,type){
    const d=parseKey(k);
    const dow=d.getDay();
    // Friday NY counts as 5M
    if(dow===5 && type==="NY") return 1;
    // Saturday / Sunday with service => 5M (including Saturday NY)
    if((dow===6 || dow===0) && hasService(type)) return 1;
    return 0;
  }
  ...
  function renderCal(){
    ...
    for(let r=0;r<6;r++){
      ...
      for(let c=0;c<7;c++){
        ...
        const badgeLabel = type==="PR"?"ΠΡ":type==="AP"?"ΑΠ":type==="NY"?"ΝΥ":type==="AD"?"ΑΔ":type==="RP"?"Ρ":"";
        html += `<div class="${cls}" data-key="${k}" data-inmonth="${inMonth?1:0}">
          <div class="dnum">${d.getDate()}</div>
          ${type? `<div class="badge ${type}">${badgeLabel}</div>`:""}
        </div>`;
      }
      html += `</div>`;
    }
    ...
    Array.from(calGrid.querySelectorAll(".calCell[data-key]")).forEach(cell=>{
      cell.addEventListener("click", ()=>{
        ...
        const cur = data[k]||"";
        if(cur===activeShift) delete data[k];
        else data[k]=activeShift;
        saveMonth(y,m,data);
        renderCal();
      });
    });
