...
(function(){
  ...
  if(clearBtn) clearBtn.addEventListener("click", ()=>{
    if(!confirm("Να καθαριστεί ο τρέχων μήνας;")) return;
    const y=view.getFullYear(), m=view.getMonth();
    try{
      localStorage.removeItem(storeKey(y,m));
    }catch(e){}
    renderCal();
  });
...
(function(){
  try{
    ...
    if(b && (b.value==='130' || b.value==='')) b.value='0';
    const fr=document.getElementById('fiveRecognize');
