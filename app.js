...
function calc() {
  ...
  const personalDiff = Number(document.getElementById('personalDiff')?.value || 0);
  ...
  const grossMain = base + fam + special + border + personalDiff;
...
// Σταθερές κρατήσεις
  ...
  setText('oPersonalDiff', euro(personalDiff));
...
function printPayslip(){
  ...
  const rowsE = [
    ...
    ['Προσωπική Διαφορά', document.getElementById('oPersonalDiff')?.textContent || '0,00€'],
...
function init(){
  ...
  document.addEventListener('input', (e)=>{
    const ids = new Set(['appointDate','status','children','after1993','border','fiveDays','nightHours','loanMtpy','personalDiff','fiveRecognize','payslipMonth','payslipYear','applyTax','neoMtpyEnabled','taxAge','taxKids']);
...
(function(){
  ...
  function hasService(type){ return !!type && type!=="RP"; }
  ...
  function stringifyTypes(arr){
    const a = Array.from(new Set((arr||[]).map(s=>String(s).trim()).filter(Boolean)));
    return a.join(',');
  }
  function hasServiceTypes(types){
    const t=parseTypes(types);
    return t.some(x=>x && x!=="RP");
  }
  function hasType(types, t){
    return parseTypes(types).includes(t);
  }

  function nightPaid(k,types){
    if(!hasType(types,"NY")) return false;
    const d=parseKey(k);
    const dow=d.getDay(); // 0 Sun,5 Fri,6 Sat
    if(dow===5 || dow===6) return false; // Fri/Sat nights count as 5M only
    return true;
  }

  function fiveCountFor(k,types){
    const d=parseKey(k);
    const dow=d.getDay();
    if(dow===5 && hasType(types,"NY")) return 1;
    if((dow===6 || dow===0) && hasServiceTypes(types)) return 1;
    return 0;
  }
  ...
  function calcCounts(y,m,data){
    ...
    Object.keys(data).forEach(k=>{
      ...
      const types=data[k]||"";
      if(nightPaid(k,types)) nights++;
      fives += fiveCountFor(k,types);
    });
    return {nights,fives};
  }
  ...
  function renderCal(){
    ...
    for(let r=0;r<6;r++){
      ...
      for(let c=0;c<7;c++){
        ...
        const badges = arr.length
          ? `<div class="badgeStack">${arr.map(t=>`<div class="badge ${t}">${labelOf(t)}</div>`).join('')}</div>`
          : "";
    ...
    Array.from(calGrid.querySelectorAll(".calCell[data-key]")).forEach(cell=>{
      cell.addEventListener("click", ()=>{
        ...
        const cur = data[k]||"";
        const arr = parseTypes(cur);
        const i = arr.indexOf(activeShift);
        if(i>=0){ arr.splice(i,1); }
        else { arr.push(activeShift); }
        const next = stringifyTypes(arr);
        if(!next) delete data[k];
        else data[k]=next;
        saveMonth(y,m,data);
        renderCal();
      });
  ...
  if(clearBtn) clearBtn.addEventListener("click", ()=>{
    const y=view.getFullYear(), m=view.getMonth();
    if(confirm("Να καθαρίσει ο μήνας;")){
      saveMonth(y,m,{});
      renderCal();
    }
  });
